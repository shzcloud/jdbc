...
public class AppendControllerData extends AppendData {
    ...
    protected List<String> content(Tgp tgp, Set<String> imports) {
        ...
        if (superEntity != null && RecordEntity.class.isAssignableFrom(superEntity)) {
            ...
            content.add("    @PostMapping(\"page\")");
            content.add("    public Response<PageInfo<Query" + className + "Vo.Vo>> page(@RequestBody @Valid PageVo<Query" + className + "Vo, Query" + className + "Vo.Vo> pageVo) {");
            content.add("        PageInfo<" + className + "> page = " + classFieldName + "Service.selectPage(pageVo.map(resVo -> FieldSetter.copy(resVo, new " + className + "())), pageVo.getData());");
            content.add("        PageInfo<Query" + className + "Vo.Vo> pageInfo = page.map(entity -> FieldSetter.copy(entity, new Query" + className + "Vo.Vo()));");
            ...
            content.add("        " + className + " entity = " + classFieldName + "Service.selectById(id);");
            content.add("        ClientFailureMsg.requireNonNull(entity, \"" + desc + "不存在\");");
            content.add("        " + className + "DetailVo detail = FieldSetter.copy(entity, new " + className + "DetailVo());");
